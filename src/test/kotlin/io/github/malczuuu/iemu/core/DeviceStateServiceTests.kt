...
class DeviceStateServiceTests {
  ...
  fun `changeState() should skip null fields and leave the modules untouched`() {
    service.changeDeviceState(DeviceStateUpdate())
  ...
  fun `subscribe methods should forward subscribers to the matching module`() {
    ...
    service.onCurrentTimeChange(onCurrentTime)
    service.onStateChange(onState)
    service.onDimmerChange(onDimmer)
    service.onTimeCounterChange(onTime)

    verify { currentTime.onCurrentTimeChange(onCurrentTime) }
    verify { light.onStateChange(onState) }
    verify { light.onDimmerChange(onDimmer) }
    verify { light.onTimeCounterChange(onTime) }
  }
