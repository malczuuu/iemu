...
class FirmwareUpdateEnablerTests {
  ...
  fun `constructor should subscribe to every change stream exposed by FirmwareService`() {
    ...
    verify { firmware.onFileChange(any()) }
    verify { firmware.onPackageUriChange(any()) }
    verify { firmware.onStateChange(any()) }
    verify { firmware.onResultChange(any()) }
    verify { firmware.onPackageVersionChange(any()) }
  }
